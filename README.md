...
実際にSarashina2.2 1B instructをブラウザ内WebLLMで動かすには、MLC-LLM形式への変換、WebGPU向けmodel libraryの用意、配信元とライセンス表記、複数端末でのロード検証が必要です。初期版では、プロダクト方針としてSarashinaを国産推奨モデルに据えつつ、動作安定性のためにWebLLM互換モデルを併用します。
...
- 国産推奨モデル: `sbintuitions/sarashina2.2-1b-instruct-v0.1`
- 日本語互換モデル: `gemma-2-2b-jpn-it-q4f32_1-MLC`
- 互換性優先モデル: `Llama-3.2-1B-Instruct-q4f32_1-MLC`
...
- Sarashina2.2 1B instructは国産推奨ソースモデルとして扱いますが、現時点ではWebLLM用のMLC変換済み成果物を同梱していません。実行時はWebLLM互換モデルへフォールバックします
