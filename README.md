...
AIまえチェックでは、WebLLMの実行モデルを `SmolLM2-360M-Instruct-q4f32_1-MLC` のみに絞っています。

理由は、モデル選択肢を増やすほど環境依存の失敗や説明コストが増えるためです。AIまえチェックは会話AIではなく、貼り付け前の「消し忘れ候補」を見つける補助ツールなので、ルールベース検出とローカル補助候補を主軸にし、WebLLMは文脈候補の確認に限定します。

SmolLM2 360Mは軽量で、WebLLM prebuiltモデルの中でも初回ロードやGPUメモリ負荷を抑えやすい選択です。日本語専用モデルではないため、WebLLMの結果は確定判定ではなく候補として扱い、ローカル補助候補とユーザー確認を組み合わせます。

日本語の人名・会社名・案件名の抽出精度はモデルだけに依存させません。WebLLMが候補を返さない場合でも、入力文に実在する敬称つき人名や `Project ...` 形式の案件名は、ローカル補助候補として確認UIに出します。これにより、軽量モデルのままでも見落としを減らし、国産モデルのMLC変換が整った後もルールベース検出と補助候補を組み合わせて使える設計にしています。

国産スクラッチモデルとしては、Sarashina2.2 1B instructを今後の候補にします。ただし、WebLLMは通常のHugging Faceモデルをそのまま読み込む仕組みではありません。実際にSarashina2.2 1B instructをブラウザ内WebLLMで動かすには、MLC-LLM形式への変換、WebGPU向けmodel libraryの用意、配信元とライセンス表記、複数端末でのロード検証が必要です。
...
- WebLLMの正式対応モデルは `SmolLM2-360M-Instruct-q4f32_1-MLC` のみです。古い設定に別モデルIDが残っていても、読み込み時に正式対応モデルへ正規化します
