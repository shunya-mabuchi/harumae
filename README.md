...
AIまえチェックでは、WebLLMの実行モデルを `Llama-3.2-1B-Instruct-q4f32_1-MLC` のみに絞っています。
...
`q4f16_1` 版はVRAM目安が軽い一方で、端末やブラウザのWebGPU機能によってはf16系の実行条件が合わない可能性があります。そのため、初期版では `q4f32_1` 版を正式対応モデルにします。
...
- WebLLMの正式対応モデルは `Llama-3.2-1B-Instruct-q4f32_1-MLC` のみです。古い設定に別モデルIDが残っていても、読み込み時に正式対応モデルへ正規化します
