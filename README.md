...
- 変換モードは `Mask` / `Generalize` / `安全な依頼文に整える`
...
- risk score / policy判定
- カテゴリ単位の確認モーダル
- `Mask` / `Generalize` / `安全な依頼文に整える` の変換モード
...
9. `Mask` / `Generalize` / `安全な依頼文に整える` のいずれかで安全化する
...
- 候補理由の日本語説明
- `Generalize` 用の抽象化候補
- `安全な依頼文に整える` 用の `safe_prompt` 生成
- ルール検出0件でも文脈ヒントがある貼り付けに対する、手動AI文脈チェック入口の提示

`safe_prompt` は、単に危険そうな文字列を削っただけの短い文章ではなく、外部AIへ貼るための自然な依頼文として生成します。たとえば顧客提案では顧客名、案件名、金額、契約前情報を抽象化し、採用・人事では候補者名、評価、給与条件を抽象化します。法務・契約では相手先、契約条件、未公開情報を抽象化し、社内情報では社内URL、組織名、内部プロジェクト名を残さない方針です。

ただし、WebLLMの出力は確定判定ではありません。生成された `safe_prompt` は必ず `packages/core` のルールベース検出で再スキャンし、APIキー、秘密鍵、JWT、`.env`、認証URLなどのSecret Guard対象や高リスク情報が残っている場合は送信できない扱いにします。
...
flowchart LR
  ...
  Modal --> Transform["Mask / Generalize / 安全な依頼文"]
  Block --> Transform
