...
const decisionLabels: Record<RiskDecisionLevel, string> = {
  safe: "安全寄り",
  low: "低",
  medium: "中",
  high: "高",
...
export function createConfirmedText(
  ...
  const selectedFindings = findings.filter((finding) => selectedFindingIds.has(finding.id));
...
function selectedFindings(findings: Finding[], selectedFindingIds: Set<string>): Finding[] {
