...
type RiskPillTone = "neutral" | "danger";
type FooterLayout = "actionsOnly" | "noteGrid";

interface SharedModalCssOptions {
  prefix: string;
...
function createRiskPillCss(prefix: string, tone: RiskPillTone, includeLevels = false): string {
...
function createFooterCss(prefix: string, layout: FooterLayout): string {
  const layoutCss =
    layout === "noteGrid"
      ? `
    display: grid;
...
${joinSelectors(options.headingSelectors)} {
  ...
  }${createFooterCss(prefix, options.footerLayout)}
  .${prefix}-button {
    min-height: 42px;
    border: 1px solid ${colors.border};
    border-radius: ${radius.content};
