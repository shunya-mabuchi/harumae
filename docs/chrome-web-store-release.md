...
Developer Dashboardへ入力する掲載情報の原本は [chrome-web-store-listing.json](./chrome-web-store-listing.json) で管理します。審査画面へそのまま貼り付けるための最終版は [chrome-web-store-submission-copy.md](./chrome-web-store-submission-copy.md) にまとめています。このセクションは人間が読みやすい確認用のドラフトです。
...
また、WebLLMを利用したAI文脈チェックにより、正規表現では拾いにくい顧客名候補、人名候補、会社名候補、案件名候補、契約・採用・給与・法務などの文脈リスク候補を確認できます。AI文脈チェックは補助的な候補提示であり、最終的な安全判定を断言するものではありません。
...
<https://github.com/shunya-mabuchi/ai-mae-check/blob/main/docs/privacy-policy.md>

紹介LPを公開した後は、LP側のプライバシーポリシー固定ページへ差し替えます。
...
スクリーンショットは、READMEに掲載している実機クロップ画像を1280x800へ整形したものを使います。アイコンとプロモーション画像はブランド訴求用の生成画像として扱います。
...
- [ ] ZIPをアップロードする
- [ ] [chrome-web-store-submission-copy.md](./chrome-web-store-submission-copy.md) を見ながらStore listingを入力する
- [ ] [chrome-web-store-submission-copy.md](./chrome-web-store-submission-copy.md) を見ながらPrivacy practicesを入力する
- [ ] Distributionを設定する
