...
このドキュメントは、AIまえチェックをChrome Web Storeへ提出するための公開前チェックリストです。
...
- 拡張名: `AIまえチェック`
- 説明: `AIに送る前に、個人情報・秘密情報・APIキーの消し忘れをブラウザ内で確認します。`
- Manifest: V3
- 権限: `storage`
- 対象サイト:
  - `https://chatgpt.com/*`
...
AIまえチェックは、ChatGPT、Claude、Geminiなどに文章を送る前に、個人情報・秘密情報・APIキー・社外秘に近い内容の消し忘れに気づくためのChrome拡張です。
...
4. 送信前確認モーダルが表示されることを確認します。
5. 「安全化して送信」またはマスク系の操作で、検出箇所がプレースホルダーに置き換わることを確認します。
